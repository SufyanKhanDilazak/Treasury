...
export default function AttarPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Get category info
      const categoryData = await getCategoryBySlug('attar')
      ...
      if (categoryData) {
        ...
        setError('Attar category not found')
      }
    } catch (error) {
      console.error('Error fetching attar products:', error)
    ...
    <div className="min-h-screen bg-[#222222] mt-16">
      <div className="w-full px-0 py-8 sm:py-12">
        ...
        >
          ...
          >
            Pure and concentrated oil-based fragrances crafted with traditional Middle Eastern techniques. 
            Experience the authentic essence of premium attar perfumes.
          </motion.p>
        </motion.div>
  ...
  whileHover={{ 
    ...
    transition: { duration: 0.3 } // Move first transition here
  }}
  initial={{ opacity: 0, y: 30 }}
  animate={{ 
    opacity: 1, 
    y: 0, 
    transition: { delay: 0.3, duration: 0.8 } // Move second transition here
  }}
>
          {isLoading ? (
            <motion.div 
            ...
            >
              ...
              >
                ...
                >
                  {filterBy === 'all' ? 'No attar products available' : `No products match "${filterBy}" filter`}
                </motion.p>
                <motion.p 

...
import { getCategoryBySlug, getProductsByCategory } from '@/sanity/lib/actions'
...
// Main Bakhoor Candles Page Component
export default function BakhoorCandlesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Get category info
    const categoryData = await getCategoryBySlug('bakhoor-candles')
    ...
    if (categoryData) {
        ...
        setError('Bakhoor Candles category not found')
      }
    } catch (error) {
      console.error('Error fetching bakhoor candles products:', error)
    ...
    <div className="min-h-screen bg-[#222222] mt-16">
      <div className="w-full px-0 py-8 sm:py-12">
        ...
        >
          ...
          >
            <motion.h1 
              className="text-4xl sm:text-5xl md:text-6xl font-black text-white mb-4 bg-gradient-to-r from-amber-400 to-orange-600 bg-clip-text text-transparent"
            ...
            <motion.div 
              className="w-24 h-1 bg-gradient-to-r from-amber-500 to-orange-500 mx-auto mb-6"
              initial={{ width: 0 }}
              animate={{ width: 96 }}
          ...
          >
            Premium bakhoor candles infused with traditional Middle Eastern fragrances. 
            Create an ambiance of luxury and warmth with our carefully crafted aromatic candles.
          </motion.p>
        </motion.div>
          ...
          {isLoading ? (
            ...
            >
              ...
              >
                ...
                >
                  {filterBy === 'all' ? 'No bakhoor candles available' : `No products match "${filterBy}" filter`}
                </motion.p>
                <motion.p 

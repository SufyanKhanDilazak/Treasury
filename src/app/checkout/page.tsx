    ...
    return (
      <div className="min-h-screen bg-white">
        <div className="container mx-auto px-4 py-20 text-center pt-24">
          <div className="max-w-md mx-auto bg-white border-[0.5px] border-[#A64D9D] rounded-xl p-8 shadow-xl">
            <ShoppingBag className="h-16 w-16 text-[#A64D9D] mx-auto mb-4" />
            <h1 className="text-xl md:text-2xl font-bold text-black mb-2">Your cart is empty</h1>
            <p className="text-black/70 mb-6 text-sm md:text-base">Add some items to your cart before checking out.</p>
            <Link href="/categories/all_product">
              <Button className="bg-[#A64D9D] hover:bg-[#D946EF] text-black px-6 py-2 rounded-lg transition-all duration-300 hover:shadow-lg">
                Continue Shopping
              </Button>
            </Link>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-3 gap-4 lg:gap-6">
            ...
            <div className="lg:col-span-1 lg:mt-20">
              <div className="bg-white border-[0.5px] border-[#A64D9D] rounded-xl p-4 lg:p-6 shadow-xl lg:sticky lg:top-6">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between text-black/70">
                    <span>Subtotal</span>
                    <span className="text-[#bd1f1f]">PKR {checkoutItems.reduce((sum, item) => sum + item.price * item.quantity, 0).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-black/70">
                    ...
                    <span className="text-[#bd1f1f]">PKR 500.00</span>
                  </div>
                  <Separator className="my-3 bg-[#A64D9D]" />
                  <div className="flex justify-between text-lg font-bold text-black">
                    <span>Total</span>
                    <span className="text-[#bd1f1f]">PKR {checkoutTotal.toFixed(2)}</span>
                  </div>
                </div>
...
function FormField({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      <Label htmlFor={props.id} className="flex items-center gap-2 text-sm font-medium text-black">
        <Icon className="h-4 w-4 text-[#A64D9D]" />
        {label}
      </Label>
      <Input
        {...props}
        className={`w-full px-3 py-2 sm:py-3 border rounded-lg bg-white text-black placeholder-black/40 transition-all duration-300 focus:outline-none focus:ring-2 ${
          error
...
function OrderItem({ item, isBuyNow = false }: { item: CartItem; isBuyNow?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-white border border-[#A64D9D]/30 rounded-lg hover:border-[#A64D9D] transition-all duration-300">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="space-y-0.5">
          {item.selectedSize && <p className="text-xs text-black/50">Size: {item.selectedSize}</p>}
          {item.selectedColor && <p className="text-xs text-black/50">Color: {item.selectedColor}</p>}
          <p className="text-sm font-medium text-[#bd1f1f]">PKR {item.price.toFixed(2)}</p>
        </div>
      </div>
      <div className="flex flex-col items-end gap-2">
        {!isBuyNow ? (
          <div className="flex items-center gap-1 bg-white border border-[#A64D9D] rounded-md">
            <Button 
              type="button" 
              variant="ghost" 
              size="sm" 
              className="h-6 w-6 sm:h-7 sm:w-7 p-0 text-black hover:bg-[#A64D9D] hover:text-black" 
              onClick={handleDecrement} 
              disabled={item.quantity <= 1}
            >
              <Minus className="h-3 w-3" />
            </Button>
            <span className="w-6 sm:w-8 text-center text-sm font-medium text-black">{item.quantity}</span>
            <Button 
              type="button" 
              variant="ghost" 
              size="sm" 
        ...
        <p className="font-semibold text-[#bd1f1f] text-sm">PKR {(item.price * item.quantity).toFixed(2)}</p>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mt-20 sm:mt-24">
        <Link href={isBuyNow ? "/cart" : "/payment-success"}>
          <Button variant="ghost" size="sm" className="gap-2 text-black/70 hover:text-black hover:bg-[#A64D9D] self-start">
            <ArrowLeft className="h-4 w-4" />
            {isBuyNow ? "Back to Product" : "Back to Cart"}
          </Button>
        </Link>
        <h1 className="text-lg sm:text-xl font-bold text-black">Place Order</h1>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        <div className="bg-white border-[0.5px] border-[#A64D9D] p-4 sm:p-6 rounded-xl shadow-xl">
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-black">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Button
            type="submit"
            disabled={loading}
            className="flex-1 h-12 text-base font-medium bg-[#A64D9D] hover:bg-[#D946EF] text-black transition-all duration-300 disabled:opacity-50 rounded-lg"
          >
            {loading ? (

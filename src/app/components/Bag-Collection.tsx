...
const CategoryBanner = memo<CategoryBannerProps>(({
  ...
  subtitle = 'Discover our curated selection of premium bags designed for the modern lifestyle',
  className = '',
}) => {
  return (
...
const BagCollection = memo(() => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const query = `*[_type == "product" && references(*[_type == "category" && title == "bag-showcase"]._id) && defined(images[0].asset->url)] | order(_createdAt desc)[0...8] {
        _id,
        name,
        price,
        onSale,
      ...
      <section className="text-center mb-12 font-montserrat">
        <div className="px-2 sm:px-6 mb-4">
          <CategoryBanner
            ...
            title="Bag Collection"
            subtitle="Discover our latest premium bag collection designed for the modern lifestyle"
          />
        </div>
...
BagCollection.displayName = 'BagCollection'
